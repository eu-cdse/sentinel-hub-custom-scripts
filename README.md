...
- **composite**: an RGB image generated by assigning a spectral product to each band of an image (like RGB ratio)
- **index**: a scalar generated by calculating a single value from one or more spectral bands and visualizing it with a palette (like NDVI)
- **regression**: an index that is calibrated to a meaningful physical quantity that has a dimension (like Leaf Area Index)
- **classification**: an operation on one or more bands where the outcome is a unique value from a pre-defined set - typically based on threshold values from one or more regresssions (like flood mapping)
