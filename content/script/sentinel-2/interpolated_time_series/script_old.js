...
var sampled_dates = sample_timestamps(start_date, end_date, 10, "day").map(
    (d) => withoutTime(d),
);
...
function increase(original_date, period, period_unit) {
    ...
    switch (period_unit) {
        case "millisecond":
            return new Date(
                date.setMilliseconds(date.getMilliseconds() + period),
            );
        case "second":
...
function evaluatePixel(samples, scenes) {
    ...
    for (var i of valid_indices) {
        fillValues(valid_samples, samples[i]);
    }

    // Calculate indices and return optimised for UINT16 format (will need unpacking)
    no_data_value = 10000;
    var [b02_interpolated, dm] = lininterp(
        sampled_dates,
        valid_dates,
        valid_samples["B02"],
        no_data_value,
    );
    var [b03_interpolated, dm] = lininterp(
        sampled_dates,
        valid_dates,
        valid_samples["B03"],
        no_data_value,
    );
    var [b04_interpolated, dm] = lininterp(
        sampled_dates,
        valid_dates,
        valid_samples["B04"],
        no_data_value,
    );
    var [b08_interpolated, dm] = lininterp(
        sampled_dates,
        valid_dates,
        valid_samples["B08"],
        no_data_value,
    );
    var [b11_interpolated, dm] = lininterp(
        sampled_dates,
        valid_dates,
        valid_samples["B11"],
        no_data_value,
    );
    var [b12_interpolated, dm] = lininterp(
        sampled_dates,
        valid_dates,
        valid_samples["B12"],
        no_data_value,
