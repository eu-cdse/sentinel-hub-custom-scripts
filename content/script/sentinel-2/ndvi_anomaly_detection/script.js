    ...
    collections.scenes.orbits = collections.scenes.orbits.filter(
        function (orbit) {
            ...
            return (
                ...
                orbitDateFrom.getFullYear() >=
                    collections.to.getFullYear() - nbPastYears
            );
        },
    );
    return collections;
...
function calculateLoss(
    ...
    if (
        ...
        return 0;
    var percentage =
        (pastIndexAverage - currentIndexAverage) / pastIndexAverage;
...
var calculateNdviAnomaly = function calculateNdviAnomaly(
    ...
    return Math.max(
        Math.min(
            indexesAverages.current - indexesAverages.past,
            pixelEvalMaxValue,
        ),
        0 - pixelEvalMaxValue,
    );
...
function calculateIndexAverages(samples, scenes, config, processSampleMethod) {
    ...
    return {
        current:
            (currentYearIndex &&
                currentYearIndex.count >=
                    config.currentIndexesMinValuesNumber &&
