...
*/
// BANDS REQUESTED: only the bands used by the active index and compositions are listed.
// When activating a commented-out index or composition below, add the bands it uses to "input".
function setup() {
	return {
		input: ["B02", "B03", "B04", "B05", "B06", "B08", "dataMask"],
		output: { bands: 4 }
	};
}
...
//var NDSI = (sample.B03-sample.B11)/(sample.B03+sample.B11);  // Discriminates WATER&SNOW x NON-WATER: standard SNOW ~ > 0.42
//var NDVI = (sample.B08-sample.B04)/(sample.B08+sample.B04);  // Discriminates VEGETATION x NON-VEG: ~ Water<0 Soil,Sand,Snow,Clouds=-.1,+.1 LowVeg=.2,.4 DenseVeg>.4
var NDWI2 = (sample.B03-sample.B08)/(sample.B03+sample.B08); // Discriminates WATER x NON-WATER: standard WATER limit ~ > 0.3
//------------------------------------------------------------------------------
...
var NATURAL_REDGE = [(sample.B04*4.0), (sample.B03*2.8+sample.B06*1.5), (sample.B02*3.5)]; // Near Natural; turbidity and algae RedEdge
//var FALSECOLOR_NIR = [(sample.B08*2.3), (sample.B03*1.0+sample.B05*2.0), (sample.B02*2.7)]; // Bluish water NIR; algae RedEdge
// B) Proposed for INVERSE SELECTION (LAND):
var NATURAL_NIR = [(sample.B04*3.0+sample.B05*1.0), (sample.B03*3.0+sample.B08*1.0), (sample.B02*3.5)] ; // Near Natural; Vegetation NIR
//var NATURAL_SWIR = [(sample.B04*2.6+sample.B12*0.8), (sample.B03*3.0+sample.B08*0.5), (sample.B02*3.0)] ; // Bare soil SWIR; Vegetation NIR
//var GEOLOGY_SWIR = [(sample.B12*2.2), (sample.B04*1.4+sample.B08*1.0), (sample.B02*2.5)] ; // Geology SWIR; Vegetation NIR
//------------------------------------------------------------------------------
...
//return (( NDSI > 0.42 ) ? EnhVERSE : EnhINVERSE).concat(sample.dataMask) ; // For SNOWY areas only
//return (( NDVI > 0.4 ) ? EnhVERSE : EnhINVERSE).concat(sample.dataMask) ; // For VEGETATION areas only
return (( NDWI2 > 0.1 ) ? EnhVERSE : EnhINVERSE).concat(sample.dataMask) ; // LAND x WATER: limit lowered to take clouds w/ water
//
}
