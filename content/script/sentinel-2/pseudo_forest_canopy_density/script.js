...
function evaluatePixel(sample) {
    if (detect_water) {
        ...
        if (ndwi > ndwi_hi) return water.concat(sample.dataMask);
    }

    let ndvi = (sample.B08 - sample.B04) / (sample.B08 + sample.B04);
    let bi_1 =
        (sample.B08 + sample.B03 + sample.B04) /
        (sample.B08 + sample.B03 - sample.B04);
    ...
    else if (ndvi > ndvi_lo) return grass_land.concat(sample.dataMask);
    else if (ndvi < ndvi_lo && bi_1 > bi_hi && si_lo > si)
        return bare_land.concat(sample.dataMask);
    else return other.concat(sample.dataMask);
}
